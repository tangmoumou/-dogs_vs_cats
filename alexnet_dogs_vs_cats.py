...
###并利用截断正态分布赋初值，采用11*11大小的窗口，将输入厚度为3的图片映射成输出为96层的feature map
#定义偏差bias
...
conv1=tf.nn.relu(tf.nn.conv2d(x_input,w_conv1,strides=[1,2,2,1],padding='VALID')+b_conv1)#2d卷积方法，输入x_input，w_conv1，
#######步长4*4，不填充，此时shape为[-1,50,50,96]
tf.summary.histogram('conv1',conv1)
'''进行第一次归一化'''
norm1=tf.nn.local_response_normalization(conv1,depth_radius,alpha,beta,bias)
tf.summary.histogram('norm1',norm1)

'''对feature map进行第一次池化'''
pool1=tf.nn.max_pool(norm1,ksize=[1,2,2,1],strides=[1,2,2,1],padding='VALID')#最大池化，ksize经验赋值，shape([-1,25,25,96])

'''进行第二次卷积'''
w_conv2=tf.Variable(tf.truncated_normal([5,5,32,64],stddev=0.1))
b_conv2=tf.Variable(tf.constant(0.1,shape=[64]))
conv2=tf.nn.relu(tf.nn.conv2d(pool1,w_conv2,strides=[1,1,1,1],padding='SAME')+b_conv2)#shape=[-1,25,25,512]
'''进行第二次归一化'''
norm2=tf.nn.local_response_normalization(conv2,depth_radius,alpha,beta,bias)
tf.summary.histogram('norm2',norm2)

'''进行第二次池化'''
pool2=tf.nn.max_pool(norm2,ksize=[1,2,2,1],strides=[1,2,2,1],padding='VALID')#shape=[-1,12,12,512]

'''第三次卷积'''
w_conv3=tf.Variable(tf.truncated_normal([3,3,64,128],stddev=0.1))
b_conv3=tf.Variable(tf.constant(0.1,shape=[128]))
conv3=tf.nn.relu(tf.nn.conv2d(pool2,w_conv3,strides=[1,1,1,1],padding='SAME')+b_conv3)#shape=[-1,12,12,768]

'''第四次卷积'''
w_conv4=tf.Variable(tf.truncated_normal([3,3,128,128],stddev=0.1))
b_conv4=tf.Variable(tf.constant(0.1,shape=[128]))
conv4=tf.nn.relu(tf.nn.conv2d(conv3,w_conv4,strides=[1,1,1,1],padding='SAME')+b_conv4)#shape=[-1,12,12,768]

'''第五次卷积'''
w_conv5=tf.Variable(tf.truncated_normal([3,3,128,128],stddev=0.1))
b_conv5=tf.Variable(tf.constant(0.1,shape=[128]))
conv5=tf.nn.relu(tf.nn.conv2d(conv4,w_conv5,strides=[1,1,1,1],padding='SAME')+b_conv5)#shape=[-1,12,12,512]

'''进行第五次池化'''
pool5=tf.nn.max_pool(conv5,ksize=[1,2,2,1],strides=[1,2,2,1],padding='VALID')#shape=[-1,6,6,512]

'''连接全连接层1'''
#定义全连接层的参数
#定义权重weight,目的是将每个图片的3维tensor特征变为1维数组特征列表，故先对池化后的feature map进行reshape
flattened=tf.reshape(pool5,[-1,12*12*128])
w_fullc1=tf.Variable(tf.truncated_normal([12*12*128,1024],stddev=0.1))
...
train_writer = tf.summary.FileWriter('alexnet_logs', sess.graph)
...
try: 
    for step in np.arange(MAX_STEP):
        ...
        if step % 2000 == 0 or (step + 1) == MAX_STEP:
            checkpoint_path = os.path.join('my_alexnet', 'model.ckpt')
